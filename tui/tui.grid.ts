...
module tui.ctrl {
	...
	export class Grid extends Control<Grid> {
		...
		private _data: tui.data.IDataProvider = null;
			...
			if (this.hasHScroll()) {
				...
				for (var i = 0; i < this._columns.length; i++) {
					this._contentWidth += Grid.colSize(
						this._columns[i].width, defaultWidth) + this._borderWidth;
				}
		...
		data(): tui.data.IDataProvider;
		data(data: tui.data.IDataProvider): Table;
		data(data?: tui.data.IDataProvider): any {
			if (data) {
				this._data = data;
				return this;
			} else {
